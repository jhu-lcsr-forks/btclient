...
/** \file main.c
    \brief An interactive demo of wam haptic capabilities.
 
    Read the code to see what you can do with it.
    
This program allows a user to test and interact with the haptic environment
...
void init_haptics(void)
{
    ...
    new_bthaptic_scene(&bth,20);
    init_state_btg(&pstate,0.002,30.0);

    /* Configure box geometry */
    init_bx_btg(&boxs[0],const_v3(p1,0.7,0.0,zorig+0.0),const_v3(p2,0.7,0.01,zorig+0.0),const_v3(p3,0.7,0.0,zorig+0.01),1.0,0.6,0.6,1);
    init_bulletproofwall(&bpwall[0],0.0,0.0,0.05,4000.0,10.0,10.0); // Set up a wall type
    //init_bulletproofwall(&bpwall[0],0.002,3000.0,0.002,3000.0,50.0,20.0);
    //for(cnt = 0;cnt < 6;cnt++) { // Apply the wall type to the sides of the box
        //init_normal_plane_bth(&objects[objectCount],&boxs[0].side[cnt],(void*)&bpwall[0],bulletproofwall_nf);
    ...
    for(cnt = 0;cnt < 4;cnt++) { // Apply the wall type to the spheres
        init_wickedwall(&wickedwalls[cnt],3000.0, 10.0,5.0,0.020,0.01); // Set up a wall type
   ...
   //zorig = 0.24; 
    ...
    for(div = 1; div <= 4; div++){
        y = -0.3125 + (div * 0.125);
        init_bx_btg(&boxs[div],const_v3(p1,0.5,y,zorig-0.25),const_v3(p2,0.5,y+0.01,zorig-0.25),const_v3(p3,0.5,y,zorig-0.24),0.1,0.025,0.1,0);
        //init_bulletproofwall(&bpwall[0],0.0,0.0,0.05,4000.0,10.0,10.0); // Set up a wall type
        // Apply the wall type to the sides of the box
        init_normal_box_bth(&objects[objectCount],&boxs[div],(void*)&bpwall[0],bulletproofwall_nf);
        addobject_bth(&bth,&objects[objectCount++]);
    }
